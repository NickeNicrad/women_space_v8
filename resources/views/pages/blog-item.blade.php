...
@section('keywords', $categoryTitle)

@section('ws_meta')
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:title" content="{{ ucwords($post->title) }}">
    <meta property="og:author" content="{{ $authorName ?: 'Nix, inc' }}">
                ...
                <div class="margin-bottom-0">
                    <div class="owl-carousel owl-nav-overlay" data-owl-items="1" data-owl-nav="true" data-owl-dots="false">
                        @foreach(explode('|', $post->images) as $image)
                            <img src="{{ Storage::url($image) }}" style="height: 480px; width: 100%; object-fit: contain;" alt="">
                        @endforeach
                    </div>
                    <div class="margin-top-30">
                        <div class="d-inline-flex">
                            <a class="font-family-tertiary font-small fw-bold uppercase" href="#">
                                {{ucfirst($categoryTitle)}}
                            </a>
                        </div>
                        <div class="d-flex gap-3 justify-content-between margin-bottom-10">
                            <div class="font-small">
                                <span>{{ date('M d, Y', strtotime($post->created_at)) }}</span>
                                <span>  <a href="#">{{ucwords($authorName)}}</a></span>
                            </div>
                            <span class="font-large d-flex gap-3">
                                <small class="font-small d-block d-lg-none">Partager</small>
                                <a target="_blank" href="http://www.facebook.com/sharer.php?s=100&amp;p[title]={{ ucwords($post->title) }}&amp;p[summary]={{ ucfirst($post->content) }}&amp;p[url]={{ url()->current() }}&amp;p[images][0]={{ asset(Storage::url(explode('|', $post->images)[0])) }}"><i class="bi bi-facebook"></i></a>
                                <a target="_blank" href="https://twitter.com/intent/tweet?text={{ ucwords($post->title) }}&amp;url={{ url()->current() }}"><i class="bi bi-twitter"></i></a>
                        ...
                        <div class="popular-post">
                            ...
                            <div>
                                <h6 class="font-small fw-medium">
                                    <a href="{{ url('actualité/' . $item->slug) }}" style="overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical;">
                                        {{ ucwords($item->title) }}
                                    </a>
                                </h6>
